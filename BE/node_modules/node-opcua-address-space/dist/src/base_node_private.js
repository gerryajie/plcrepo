...
const errorLog = (0, node_opcua_debug_1.make_errorLog)("address-space:BaseNode");
const doTrace = (0, node_opcua_debug_1.checkDebugFlag)("INSTANTIATE");
const traceLog = errorLog;
const warningLog = (0, node_opcua_debug_1.make_warningLog)("address-space:BaseNode");
...
function BaseNode_References_toString(options) {
    ...
    options.add(`${options.padding + chalk_1.default.yellow("    references                : ")}  length =${_private._referenceIdx.size}`);
...
function VariableOrVariableType_toString(options) {
    ...
    if (this.minimumSamplingInterval !== undefined) {
        options.add(`${options.padding + chalk_1.default.yellow(" minimumSamplingInterval      : ")} ${this.minimumSamplingInterval.toString()} ms`);
    }
    if (this.arrayDimensions) {
        options.add(`${options.padding + chalk_1.default.yellow(" arrayDimension               : ")} [${this.arrayDimensions.join(",").toString()} ]`);
    }
}
/**
 *
...
function _cloneInterface(nodeToClone, _newParent, node, optionalFilter, extraInfo, browseNameMap) {
    // c8 ignore next
    doTrace &&
...
function __clone_interfaces_related_references(nodeToClone, newParent, copyAlsoModellingRules, optionalFilter, extraInfo, browseNameMap) {
    // find all references that derives from the HasInterface
    const hasInterfaceRef = nodeToClone.findReferencesEx("HasInterface", node_opcua_data_model_1.BrowseDirection.Forward);
    if (hasInterfaceRef.length === 0)
        return;
    for (const ref of hasInterfaceRef) {
        let typeDefinitionNode = reference_impl_1.ReferenceImpl.resolveReferenceNode(nodeToClone.addressSpace, ref);
        if (!typeDefinitionNode) {
            continue;
        }
        if (doTrace)
            traceLog(extraInfo?.pad(), chalk_1.default.blueBright(newParent.browseName.toString(), "-----> Exploring interface ", typeDefinitionNode.browseName.toString()));
        while (typeDefinitionNode) {
            // c8 ignore next
            doTrace &&
                traceLog(extraInfo?.pad(), chalk_1.default.blueBright(newParent.browseName.toString(), "-----> Exploring ", typeDefinitionNode.browseName.toString()));
            __clone_children_references(typeDefinitionNode, newParent, copyAlsoModellingRules, optionalFilter, extraInfo, browseNameMap);
            typeDefinitionNode = typeDefinitionNode.subtypeOfObj;
        }
    }
}
...
function _makeReferenceDescription(addressSpace, reference, resultMask) {
    ...
    if (!obj) {
        ...
        const obj2 = obj;
        const objOrVar = (obj2.nodeClass === node_opcua_data_model_1.NodeClass.Object || obj2.nodeClass === node_opcua_data_model_1.NodeClass.Variable)
            ? obj2 : null;
        ...
        data = {
            ...
            nodeId: obj.nodeId,
            referenceTypeId: resultMask & node_opcua_data_model_1.ResultMask.ReferenceType ? referenceTypeId : null,
            typeDefinition: resultMask & node_opcua_data_model_1.ResultMask.TypeDefinition ? objOrVar?.typeDefinition : undefined
        };
    }
    if (data.typeDefinition === null) {
        data.typeDefinition = node_opcua_nodeid_1.ExpandedNodeId.nullNodeId;
    }
