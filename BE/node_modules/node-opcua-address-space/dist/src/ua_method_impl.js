...
class UAMethodImpl extends base_node_impl_1.BaseNodeImpl {
    ...
    readAttribute(context, attributeId, indexRange, dataEncoding) {
        ...
        switch (attributeId) {
            case node_opcua_data_model_1.AttributeIds.Executable:
                options.value = { dataType: node_opcua_variant_1.DataType.Boolean, value: this.getExecutableFlag(context) };
                options.statusCode = node_opcua_status_code_1.StatusCodes.Good;
                break;
            case node_opcua_data_model_1.AttributeIds.UserExecutable:
                options.value = { dataType: node_opcua_variant_1.DataType.Boolean, value: this.getUserExecutableFlag(context) };
                options.statusCode = node_opcua_status_code_1.StatusCodes.Good;
                break;
            default:
    ...
    bindMethod(async_func) {
        ...
        if (async_func.length === 2) {
            async_func = (0, node_util_1.callbackify)(async_func);
        }
    ...
    execute(object, inputArguments, context, callback) {
        ...
        if (object.nodeClass !== node_opcua_data_model_1.NodeClass.Object && object.nodeClass !== node_opcua_data_model_1.NodeClass.ObjectType) {
            warningLog("Method " +
                ...
                node_opcua_data_model_1.NodeClass[context.object?.nodeClass]);
        ...
        const coercedInputArguments = inputArguments;
        ...
        const runInterceptorsAndExecute = async () => {
            ...
            for (const interceptor of interceptors) {
                ...
                try {
                    ...
                    if (err instanceof Error) {
                        warningLog(chalk_1.default.red("ERR in method interceptor"), err.message);
                    }
            ...
            try {
                this._asyncExecutionFunction?.call(this, coercedInputArguments, context, (err, callMethodResult) => {
                    if (err) {
                    ...
                    callMethodResult.inputArgumentResults =
                        callMethodResult.inputArgumentResults?.length === coercedInputArguments?.length
                            ? callMethodResult.inputArgumentResults
                            : coercedInputArguments?.map(() => node_opcua_status_code_1.StatusCodes.Good);
                    ...
                    try {
                        ...
                        if (afterCallErr instanceof Error) {
                            warningLog(chalk_1.default.red("ERR in afterCall listener"), afterCallErr.message);
                        }
                    }
    ...
    _getArguments(name) {
        ...
        (0, node_opcua_assert_1.assert)(argsVariable.nodeClass === node_opcua_data_model_1.NodeClass.Variable);
