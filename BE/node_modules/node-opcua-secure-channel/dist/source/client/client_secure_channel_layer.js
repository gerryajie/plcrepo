 ...
 */
class ClientSecureChannelLayer extends node_events_1.EventEmitter {
    static g_counter = 0;
    ...
    constructor(options) {
        ...
        this.endpointUrl = "";
        const g = global;
        if (g.hasResourceLeakDetector && !g.ResourceLeakDetectorStarted) {
    ...
    getCertificateChain() {
        return this.#parent ? this.#parent.getCertificateChain() : [];
    }
    getCertificate() {
    ...
    create(endpointUrl, callback) {
        ...
        if (this.securityMode !== node_opcua_service_secure_channel_1.MessageSecurityMode.None) {
            ...
            if (!this.#serverCertificate) {
                wrapperCallback(new Error("ClientSecureChannelLayer#create : expecting a server certificate when securityMode is not None"));
                return;
            }
            // take the opportunity of this async method to perform some async pre-processing
            if (!this.#receiverPublicKey) {
                const chain = Array.isArray(this.#serverCertificate) ? this.#serverCertificate : [this.#serverCertificate];
                (0, node_opcua_assert_1.assert)(chain.length === 1);
                (0, web_1.extractPublicKeyFromCertificate)(chain[0], (err, publicKey) => {
                    /* c8 ignore next */
                    if (err) {
                        wrapperCallback(err);
                        return;
                    ...
                    this.#receiverPublicKey = (0, node_crypto_1.createPublicKey)(publicKey);
    ...
    abortConnection(callback) {
        ...
        const step1_abortBackoff = () => {
            if (!this.__call)
                return Promise.resolve();
            if (typeof this.__call !== "number") {
                return new Promise((resolve) => {
                    this.__call.once("abort", () => resolve());
        ...
        const step2_disconnectPendingTransport = () => {
        ...
        const step3_disconnectTransport = () => {
            const transport = this.#_transport;
            if (!transport)
                return Promise.resolve();
            return new Promise((resolve) => {
    ...
    close(callback) {
        ...
        this.cancelPendingTransactions(( /* err?: Error */) => {
            ...
            if (!this.#_transport || this.#_transport.isDisconnecting() || !this.#_isOpened) {
                this.abortConnection(() => {
                    this.dispose();
                    callback(new Error("Transport disconnected or channel not opened"));
                });
                return;
            }
    ...
    #_install_message_builder() {
        ...
        /* c8 ignore next */
        if (node_opcua_transport_1.doTraceChunk) {
    ...
    #_on_message_received(response, msgType, _securityHeader, requestId) {
        /* c8 ignore next */
        ...
        if (utils_1.doPerfMonitoring) {
            ...
            requestData.startReceivingTick = this.#messageBuilder?._tick1 || 0;
        }
        requestData.bytesRead = this.#messageBuilder?.totalMessageSize || 0;
        /* c8 ignore next */
        if (utils_1.doPerfMonitoring) {
    ...
    #_build_client_nonce() {
        ...
        return (0, node_crypto_1.randomBytes)(cryptoFactory.symmetricKeyLength);
    }
    #_send_open_secure_channel_request(isInitial, callback) {
        ...
        this.#_performMessageTransaction(msgType, msg, (err, response) => {
            // c8 ignore next
            if (response?.responseHeader && response.responseHeader.serviceResult !== node_opcua_status_code_1.StatusCodes.Good) {
            ...
            if (!err && response) {
                ...
                if (this.securityMode !== node_opcua_service_secure_channel_1.MessageSecurityMode.None) {
                    ...
                    if (!this.#messageBuilder?.securityPolicy) {
                        return callback(new Error(" Invalid security policy"));
                    }
    ...
    #_backoff_completion(err, lastError, transport, callback) {
        ...
        if (this.__call && typeof this.__call !== "number") {
            doDebug && debugLog(" aborting backoff process Retry =", this.__call.getNumRetries());
        ...
        else if (typeof this.__call === "number") {
            this.__call = null;
        }
        if (err) {
    ...
    #_establish_connection(transport, endpointUrl, callback) {
        ...
        const callObj = backoff.call(connectFunc, completionFunc);
        this.__call = callObj;
        if (this.#connectionStrategy.maxRetry >= 0) {
        ...
        callObj.on("backoff", onBackoffFunc);
        callObj.on("abort", () => {
            doDebug && debugLog(chalk_1.default.bgWhite.cyan(` abort #   after ${callObj.getNumRetries()} retries.`));
            // Do something when backoff starts, e.g. show to the
            // user the delay before next reconnection attempt.
            this.emit("abort");
    ...
    #_make_timeout_callback(request, callback, timeout) {
        ...
        const modified_callback = (err, response) => {
            ...
            (0, node_opcua_assert_1.assert)(!err || node_util_1.types.isNativeError(err));
        ...
        switch (this.securityMode) {
            ...
            case node_opcua_service_secure_channel_1.MessageSecurityMode.SignAndEncrypt: {
                ...
                const evaluateReceiverThumbprint = () => {
                    ...
                    if (!this.#serverCertificate) {
                        throw new Error("Server certificate is null");
                    }
                    const chain = Array.isArray(this.#serverCertificate) ? this.#serverCertificate : [this.#serverCertificate];
                    (0, node_opcua_assert_1.assert)(chain.length === 1);
                    const receiverCertificateThumbprint = (0, common_1.getThumbprint)(chain[0]);
                ...
                const securityHeader = new services_1.AsymmetricAlgorithmSecurityHeader({
                     ...
                     */
                    senderCertificate: (0, web_1.combine_der)(partialCertificateChain) // certificate of the private key used to sign the message
                });
                /* c8 ignore next */
    ...
    #_sendSecureOpcUARequest(msgType, request) {
        const evaluateChunkSize = () => {
            ...
            if (this.#_transport?.parameters?.sendBufferSize) {
