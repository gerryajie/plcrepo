...
function _get_parent_type_and_path(originalObject) {
    ...
    if (parents.length > 1) {
        ...
        const parentTypes = parents.filter((p) => {
            ...
            return n && (n.nodeClass === node_opcua_data_model_1.NodeClass.ObjectType || n.nodeClass === node_opcua_data_model_1.NodeClass.VariableType);
        });
        if (parentTypes.length === 1) {
            return { parentType: addressSpace.findNode(parentTypes[0].nodeId), path: [originalObject.browseName] };
        }
...
function findNonHierarchicalReferences(originalObject) {
    ...
    const _addressSpace = originalObject.addressSpace;
