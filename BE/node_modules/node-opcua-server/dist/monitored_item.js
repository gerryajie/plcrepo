...
function _validate_parameters(monitoringParameters) {
    ...
    (0, node_opcua_assert_1.assert)(Object.hasOwn(monitoringParameters, "clientHandle"));
    (0, node_opcua_assert_1.assert)(Object.hasOwn(monitoringParameters, "samplingInterval"));
...
function valueHasChanged(newDataValue, oldDataValue, deadbandType, deadbandValue) {
    ...
    (0, node_opcua_assert_1.assert)(oldDataValue instanceof node_opcua_data_value_1.DataValue);
    switch (deadbandType) {
        case node_opcua_service_subscription_1.DeadbandType.None:
            ...
            return (0, node_opcua_service_subscription_1.isOutsideDeadbandNone)(oldDataValue.value, newDataValue.value);
        case node_opcua_service_subscription_1.DeadbandType.Absolute:
            // AbsoluteDeadband
            return (0, node_opcua_service_subscription_1.isOutsideDeadbandAbsolute)(oldDataValue.value, newDataValue.value, deadbandValue);
        default: {
            // Percent_2    PercentDeadband (This type is specified in Part 8).
            (0, node_opcua_assert_1.assert)(deadbandType === node_opcua_service_subscription_1.DeadbandType.Percent);
            ...
            if (euRangeNode && euRangeNode.nodeClass === node_opcua_data_model_1.NodeClass.Variable) {
                ...
                return (0, node_opcua_service_subscription_1.isOutsideDeadbandPercent)(oldDataValue.value, newDataValue.value, deadbandValue, rangeVariant.value);
            }
            else {
...
function apply_dataChange_filter(newDataValue, oldDataValue) {
    /* c8 ignore next */
    if (!this.filter || !(this.filter instanceof node_opcua_service_subscription_1.DataChangeFilter)) {
    ...
    if (doDebug) {
        try {
            debugLog("filter pass ?", node_opcua_service_subscription_1.DataChangeTrigger[trigger], this.oldDataValue?.toString(), newDataValue.toString());
            if (trigger === node_opcua_service_subscription_1.DataChangeTrigger.Status ||
                trigger === node_opcua_service_subscription_1.DataChangeTrigger.StatusValue ||
                trigger === node_opcua_service_subscription_1.DataChangeTrigger.StatusValueTimestamp) {
                debugLog("statusCodeHasChanged ", statusCodeHasChanged(newDataValue, oldDataValue));
            }
            if (trigger === node_opcua_service_subscription_1.DataChangeTrigger.StatusValue || trigger === node_opcua_service_subscription_1.DataChangeTrigger.StatusValueTimestamp) {
            ...
            if (trigger === node_opcua_service_subscription_1.DataChangeTrigger.StatusValueTimestamp) {
    ...
    switch (trigger) {
        case node_opcua_service_subscription_1.DataChangeTrigger.Status: {
            //
            //              Status
        ...
        case node_opcua_service_subscription_1.DataChangeTrigger.StatusValue: {
        ...
        default: {
            ...
            (0, node_opcua_assert_1.assert)(trigger === node_opcua_service_subscription_1.DataChangeTrigger.StatusValueTimestamp);
...
function apply_filter(newDataValue) {
    ...
    if (this.filter instanceof node_opcua_service_subscription_1.DataChangeFilter) {
...
class MonitoredItem extends events_1.EventEmitter {
    ...
    constructor(options) {
        super();
        (0, node_opcua_assert_1.assert)(Object.hasOwn(options, "monitoredItemId"));
    ...
    _on_opcua_event(eventData) {
        ...
        const addressSpace = eventData.getEventDataSource().addressSpace;
    ...
    __acquireInitialValue(sessionContext, callback) {
        ...
        // one already
        (0, node_opcua_assert_1.assert)(this.itemToMonitor.attributeId === node_opcua_data_model_1.AttributeIds.Value);
    ...
    __start_sampling(recordInitialValue) {
        ...
        if (this.itemToMonitor.attributeId !== node_opcua_data_model_1.AttributeIds.Value) {
    ...
    _makeDataChangeNotification(dataValue) {
        ...
        if (this.filter && this.filter instanceof node_opcua_service_subscription_1.DataChangeFilter) {
            if (this.filter.trigger === node_opcua_service_subscription_1.DataChangeTrigger.Status) {
                /** */
            }
        }
